...
queue Q1(4000000);//kolejka dla punktow czekajacych na przegladniecie nieparzystych
queue Q2(4000000);//parzyste
...
main()//wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
{
    ...
    for(int i=0; i<z; i++)
    {
        ...
        point A, W1(-1,-1), W2(-1,-1);
        int x, y, a, b;
        //bool cheese_found=0;
        while(!Q2.empty())//dopoki zadna mysz nie znalazla sera, parzyste
        {
            A=Q2.dequeue(); x=A.getX(); y=A.getY();
            //cout<<x<<" "<<y<<endl;
            if(M[x][y]==3)
            {
                W2=V[x][y];
            ...
            for(int j=0; j<4; j++)
            {
                if( (V[x+k][y+l].getX()==0 or V[x+k][y+l].getX()%2==1) and (M[x+k][y+l]==0 or M[x+k][y+l]==3))
                {
                    //cout<<x+k<<"::"<<y+l<<endl;
                    A.set(x+k,y+l);
                    Q2.enqueue(A);
        ...
        while(!Q1.empty())//dopoki zadna mysz nie znalazla sera, parzyste
        {
            A=Q1.dequeue(); x=A.getX(); y=A.getY();
            //cout<<x<<" "<<y<<endl;
            if(M[x][y]==3)
            {
                W1=V[x][y];
            ...
            for(int j=0; j<4; j++)
            {
                if( V[x+k][y+l].getX()%2==0 and M[x+k][y+l]!=2)
                {
                    //cout<<x+k<<"::"<<y+l<<endl;
                    A.set(x+k,y+l);
                    Q1.enqueue(A);
        ...
        if(W1.getY()<W2.getY() and W1.getY()>0) W1.print();
        else if(W2.getY()>0) W2.print();
        else W1.print();
    }
...
void read(int n, int m)
{
    ...
    for(int i=1; i<n+1; i++)
    {
        ...
        for(int j=1; j<m+1; j++)//wczytujemy wiersz
        {
            ...
            {
                A.set(i,j);
                if(a%2==1) Q1.enqueue(A); else Q2.enqueue(A);
                V[i][j].set(a,0);
            }
            else V[i][j].set(0,0);
            M[i][j]=a;
